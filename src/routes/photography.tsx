      ...
      <EditorialShell title={<span className="hidden"></span>} intro="" eyebrow="">
          ...
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
            {data.map((p) => (
              <div 
                ...
                /* Mobile Touch Animation Added Here (active:scale-[0.98]) */
                className="break-inside-avoid relative group rounded-2xl overflow-hidden bg-[#050505] cursor-pointer border border-white/5 transition-all duration-500 hover:border-cyan-500/50 hover:shadow-[0_0_30px_rgba(34,211,238,0.25)] md:hover:-translate-y-2 active:scale-[0.98]"
              >
                {isVideoUrl(p.cover_image) ? (
                  <video 
                    src={p.cover_image} autoPlay loop muted playsInline controlsList="nodownload" onContextMenu={(e) => e.preventDefault()}
                    className="w-full h-auto object-cover group-hover:scale-[1.05] transition-transform duration-700 ease-out"
                  />
                ) : (
                  <img 
                    src={p.cover_image} alt={p.title} onContextMenu={(e) => e.preventDefault()}
                    className="w-full h-auto object-cover group-hover:scale-[1.05] transition-transform duration-700 ease-out" 
                  />
                )}
                
                {/* Mobile View: Visible Gradient, Laptop View: Hidden till hover */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10 pointer-events-none"></div>
                
                <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5 z-20 pointer-events-none">
                  {/* Mobile View: Visible Text, Laptop View: Slides up on hover */}
                  <div className="transform translate-y-0 opacity-100 md:translate-y-4 md:opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-out">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-32 border-t border-white/10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* Services Box: Mobile Touch Animation Added (active:scale-[0.98]) */}
            <div className="group relative p-6 md:p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-cyan-500/50 md:hover:-translate-y-3 hover:bg-white/[0.04] hover:shadow-[0_20px_40px_-15px_rgba(34,211,238,0.2)] active:scale-[0.98] transition-all duration-500 overflow-hidden">
            ...
            <div className="group relative p-6 md:p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-indigo-500/50 md:hover:-translate-y-3 hover:bg-white/[0.04] hover:shadow-[0_20px_40px_-15px_rgba(99,102,241,0.2)] active:scale-[0.98] transition-all duration-500 overflow-hidden lg:mt-8">
            ...
            <div className="group relative p-6 md:p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-purple-500/50 md:hover:-translate-y-3 hover:bg-white/[0.04] hover:shadow-[0_20px_40px_-15px_rgba(168,85,247,0.2)] active:scale-[0.98] transition-all duration-500 overflow-hidden">
            ...
            <div className="group relative p-6 md:p-8 rounded-3xl bg-white/[0.02] border border-white/5 hover:border-rose-500/50 md:hover:-translate-y-3 hover:bg-white/[0.04] hover:shadow-[0_20px_40px_-15px_rgba(244,63,94,0.2)] active:scale-[0.98] transition-all duration-500 overflow-hidden lg:mt-8">
          ...
          >
            <div className="md:w-3/5 overflow-hidden bg-black/50 flex items-center justify-center">
              {isVideoUrl(selectedPhoto.cover_image) ? (
                <video 
                  src={selectedPhoto.cover_image} autoPlay loop controls controlsList="nodownload" onContextMenu={(e) => e.preventDefault()}
                  className="w-full h-auto max-h-[50vh] md:max-h-[90vh] object-contain"
                />
              ) : (
                <img 
                  src={selectedPhoto.cover_image} alt={selectedPhoto.title} onContextMenu={(e) => e.preventDefault()}
                  className="w-full h-auto max-h-[50vh] md:max-h-[90vh] object-contain" 
                />
              )}
            </div>
